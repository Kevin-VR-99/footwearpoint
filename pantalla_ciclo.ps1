# ============================================================
#  FootwearPoint - Paquete C / E10: pantalla de Ciclo de compra
#  1) Componente Livewire  components\ciclo\<rayo>index.blade.php
...
#     powershell -ExecutionPolicy Bypass -File .\pantalla_ciclo.ps1
# ============================================================
...
Escribir "resources\views\components\ciclo\$($rayo)index.blade.php" $componente
...
"@
    ...
    if ($panel.Contains($ancla)) {
        $panel = $panel.Replace($ancla, $item + $ancla)
        $utf8SinBom = New-Object System.Text.UTF8Encoding($false)
        [System.IO.File]::WriteAllText($rutaPanel, $panel, $utf8SinBom)
        Write-Host "actualizado  panel.blade.php (item Ciclo de compra agregado)" -ForegroundColor Green
    } else {
    ...
    Write-Host "sin cambios  panel.blade.php (el item ya estaba)" -ForegroundColor DarkGray
}
...
if ($web -notmatch "ciclo\.index") {
    $bloque = "`r`nRoute::livewire('/ciclo', 'ciclo.index')`r`n" +
              "    ->name('ciclo.index')`r`n" +
              "    ->middleware('auth');`r`n"

    $utf8SinBom = New-Object System.Text.UTF8Encoding($false)
    [System.IO.File]::WriteAllText($rutaWeb, $web.TrimEnd() + "`r`n" + $bloque, $utf8SinBom)
    Write-Host "actualizado  routes\web.php (ruta de ciclo agregada)" -ForegroundColor Green
} else {
...
Write-Host ""
Write-Host "Y entra a http://localhost:8000/ciclo" -ForegroundColor Cyan
